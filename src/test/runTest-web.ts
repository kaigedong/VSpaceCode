...
async function downloadWhichKey() {
  ...
  const data = (await httpsGetJson(
    "https://api.github.com/repos/VSpaceCode/vscode-which-key/releases/latest"
  )) as any;
  ...
  const vscodeTestDepDir = path.resolve(
    process.cwd(),
    ".vscode-test-dependency"
  );
...
function getExtensionPaths() {
  return Promise.all([downloadWhichKey()]);
}
